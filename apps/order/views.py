...
class CartView(LoginRequiredMixin, ListViewBreadcrumbMixin):
    ...
    def get_queryset(self):
        return Cart.objects.filter(user=self.request.user).prefetch_related('product').prefetch_related(
            'product__images')
...
class ClearCartView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        messages.success(request, 'Кошик пустий')
        return redirect('order:cart')
...
class CartUpdateView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        if cart.quantity > cart.product.quantity:
            messages.add_message(request, messages.ERROR, f'На складі недостатньо товару {cart.product.name}',
                                 extra_tags='danger')
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.error(request,
                           f'Помилка зміни кількості товару: {[error for error in form.errors.values()][0][0]}',
                           extra_tags='danger')
